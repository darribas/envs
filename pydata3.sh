...
conda-env remove -y -n pydata3

conda create -y -n pydata3 python=3.5.*

conda install -c conda-forge -n pydata3 -y \
    'bokeh=0.11' \
    'cartopy=0.14*' \
    'cython=0.23.*' \
    'dask=0.8*' \
...
conda install -y -n pydata3 -c anaconda-nb-extensions nbpresent nbbrowserpdf
